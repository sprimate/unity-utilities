...
namespace HitTrax.UnityUtilities
{
    ...
    {
        public static Safe<float> X(this Safe<Vector3> optV3) => SafeFunctions.Select(optV3, v3 => v3.x);

        public static Safe<float> Y(this Safe<Vector3> optV3) => SafeFunctions.Select(optV3, v3 => v3.y);

        public static Safe<float> Z(this Safe<Vector3> optV3) => SafeFunctions.Select(optV3, v3 => v3.z);

        public static Safe<Vector3> Add(this Safe<Vector3> v3, Safe<Vector3> val) => val.SelectOut(v => v3.Add(v), () => new Safe<Vector3>());

        public static Safe<Vector3> Add(this Safe<Vector3> v3, Vector3 val) => v3.SelectOut(v => v.Add(val), () => new Safe<Vector3>());

        public static Safe<Vector3> AddX(this Safe<Vector3> v3, float val) => v3.SelectOut(v => v.AddX(val), () => new Safe<Vector3>());

        public static Safe<Vector3> AddY(this Safe<Vector3> v3, float val) => v3.SelectOut(v => v.AddY(val), () => new Safe<Vector3>());

        public static Safe<Vector3> AddZ(this Safe<Vector3> v3, float val) => v3.SelectOut(v => v.AddZ(val), () => new Safe<Vector3>());

        public static Vector3 AddX(this Vector3 v3, float val) => v3.Add(new Vector3(val, 0, 0));

        public static Vector3 AddY(this Vector3 v3, float val) => v3.Add(new Vector3(0, val, 0));

        public static Vector3 AddZ(this Vector3 v3, float val) => v3.Add(new Vector3(0, 0, val));

        public static Safe<Vector3> InsertX(this Safe<Vector3> v3, float val) => v3.SelectOut(v => v.InsertX(val), () => new Safe<Vector3>());

        public static Safe<Vector3> InsertY(this Safe<Vector3> v3, float val) => v3.SelectOut(v => v.InsertY(val), () => new Safe<Vector3>());

        public static Safe<Vector3> InsertZ(this Safe<Vector3> v3, float val) => v3.SelectOut(v => v.InsertZ(val), () => new Safe<Vector3>());

        public static Vector3 InsertX(this Vector3 v3, float val) => new Vector3(val, v3.y, v3.z);

        public static Vector3 InsertY(this Vector3 v3, float val) => new Vector3(v3.x, val, v3.z);

        public static Vector3 InsertZ(this Vector3 v3, float val) => new Vector3(v3.x, v3.y, val);

        public static float GetFacingAngle(this Component c, Component obj, out float cos) => GetFacingAngle(c, obj.transform.position, out cos);

        public static float GetFacingAngle(this Component c, Vector3 obj, out float cos)
            => Mathf.Acos(cos = c.transform.position.GetFacingAngle(c.transform.forward, obj, out cos)) * Mathf.Rad2Deg;

        public static float GetFacingAngle(this Vector3 v3, Vector3 forwardAngle, Vector3 obj, out float cos)
           => Mathf.Acos(cos = GetCosFacingAngle(v3, forwardAngle, obj)) * Mathf.Rad2Deg;

        public static float GetCosFacingAngle(this Vector3 v3, Vector3 forwardAngle, Vector3 obj)
        {
            //Debug.Log($"v3 {v3} foward {fowardAngle} obj {obj} normalized {(obj - v3).normalized} dot { Vector3.Dot((obj - v3).normalized, fowardAngle)}");
            return GetFacingDot(v3, forwardAngle, obj);
        }

        public static float GetFacingDot(this Vector3 v3, Vector3 forwardAngle, Vector3 obj)
            => Vector3.Dot((obj - v3).normalized, forwardAngle);

        public static bool IsInView(this Transform cam, Vector3 obj, float cutoffAngle)
            => GetFacingAngle(cam.transform, obj, out float cos) < cutoffAngle
                || Mathf.Approximately(cos, 1);

        public static int RandVal(int min, int max, int seed)
        {
            UnityEngine.Random.InitState(seed);
            return RandVal(min, max);
        }

        public static int RandVal(int min, int max) => UnityEngine.Random.Range(min, max);

        public static float RandVal(float min, float max) => UnityEngine.Random.Range(min, max);

        //static public int RandVal(this Range range) => RandVal(range.Min, range.Max);

        //static public float RandVal(this FloatRange range) => RandVal(range.Min, range.Max);

        public static bool RandIsGreaterThanTarget(this float target, float min, float max)
            => RandVal(min, max) > target;

        public static bool RandIsLessThanTarget(this float target, float min, float max)
            => RandVal(min, max) < target;

        public static Vector3 Abs(this Vector3 v3) => new Vector3(Mathf.Abs(v3.x), Mathf.Abs(v3.y), Mathf.Abs(v3.z));

        public static Vector3Int AsVector3Int(this Vector3 v3) => new Vector3Int((int)v3.x, (int)v3.y, (int)v3.z);

        public static Vector3 MaskX(this Vector3 v) => new Vector3(0f, v.y, v.z);

        public static Vector3 MaskY(this Vector3 v) => new Vector3(v.x, 0f, v.z);

        public static Vector3 MaskZ(this Vector3 v) => new Vector3(v.x, v.y, 0f);

        // NOTE: If a value is being modified, that should be clear, or it shouldn't be modified at all
        public static Vector3 Add(this Vector3 item, Vector3 val)
            => item = new Vector3(item.x + val.x, item.y + val.y, item.z + val.z);

        public static Vector2 RemoveY(this Vector3 v3) => new Vector2(v3.x, v3.z);

        public static Vector2 RemoveZ(this Vector3 v3) => new Vector2(v3.x, v3.y);

        public static Vector3 Add(this Vector3 item, float val) => item.Add(new Vector3(val, val, val));

        // Maybe change this to modify to make it clear that it is being modified?
        public static Vector3 Add(this Vector3 item, float x, float y) => item = new Vector3(item.x + x, item.y + y);

        public static float CrossProduct2D(Vector2 v1, Vector2 v2) => v1.x * v2.y - v1.y * v2.x;

        public static Vector2 ConvertXZToV2(this Vector3 v3) => new Vector2(v3.x, v3.z);

        public static float HorizontalAngle(this Vector3 vel)
        {
            Vector2 v = new Vector2(vel.x, vel.z);
            float rval = (Mathf.Rad2Deg * (float)Math.Asin(v.normalized.x));
            // Velo is in opposite dirrection, invert the result. Right is positive, left is negative
            if (vel.z < 0)
            {
                return -1 * rval;
            }
        ...
        public static bool ValidFloat(this float value)
           => (!Double.IsNaN(value) && !Double.IsInfinity(value));

        public static float Elevation(this Vector3 vel)
            => (Mathf.Rad2Deg * (float)Math.Asin(vel.normalized.y));
        public static float AngleBetweenVectors(Vector3 v1, Vector3 v2)
        {
            Vector3 cross = Vector3.Cross(v1, v2);
            var dot = Vector3.Dot(v1, v2);
            return (float)(Math.Atan2(cross.magnitude, dot) * 180 / Math.PI);
        }

        public static Vector3 MPH2MPS(this Vector3 mph)
            => mph * 0.44704f;

        public static double MPH2MPS(this double mph)
            => mph * 0.44704f;

        public static float MPH2MPS(this float mph)
            => mph * 0.44704f;

        public static float MM2IN(float val)
        {
            return val * 0.0393701f;
        }

        public static float M2IN(float val)
        {
            return val / 0.0254f;
        }

        public static float MPS2KPH(float val)
        {
            return val * 3.6f;
        }

        public static float MPS2MHP(float val)
        {
            return val / 0.44704f;
        }

        public static float IN2M(float val)
        {
            return val * 0.0254f;
        }

        public static float MetersPS_To_MilesPH(this float mps)
            => mps * 2.23694f;

        public static float MetersToFeet(this float meters) => meters * 3.28084f;

        public static Safe<GameObject> GetNearest(this IEnumerable<GameObject> gameObjects, Vector3 target)
        {
            float minDistance = float.MaxValue;
            Safe<GameObject> nearest = None;
        ...
        public static Safe<GameObject> GetFurthest(this IEnumerable<GameObject> gameObjects, Vector3 target)
        {
            float maxDistance = float.MinValue;
            Safe<GameObject> furthest = None;
        ...
        // NOTE: This can be move to CoreUtilities since it doesn't refernece Unity
        public static float Normalize(this float value, float min, float max)
            => Mathf.Approximately(max, min) ? 0 : Mathf.Clamp01((value - min) / (max - min));


        public static Safe<(float, Vector3)> PredictLanding(Vector3 currentPosition, Vector3 initialVelocity, Vector3 gravity, float groundLevelY)
        {
            // Break down the vertical (Y-axis) motion
            float initialHeight = currentPosition.y;
            float verticalVelocity = initialVelocity.y;
            float verticalAcceleration = gravity.y;
            float verticalDisplacement = initialHeight - groundLevelY;

            // Solve the quadratic equation for time:
            // 0.5 * g * t^2 + v * t + (y0 - groundY) = 0
            float quadraticA = 0.5f * verticalAcceleration;
            float quadraticB = verticalVelocity;
            float quadraticC = verticalDisplacement;

            float discriminant = quadraticB * quadraticB - 4 * quadraticA * quadraticC;

            if (discriminant < 0)
            {
                Debug.LogWarning("No real solution: The object will not reach the ground.");
                return None;
            }

            float sqrtDiscriminant = Mathf.Sqrt(discriminant);
            float timeToImpact1 = (-quadraticB + sqrtDiscriminant) / (2 * quadraticA);
            float timeToImpact2 = (-quadraticB - sqrtDiscriminant) / (2 * quadraticA);

            float timeUntilLanding = Mathf.Max(timeToImpact1, timeToImpact2);

            if (timeUntilLanding < 0)
            {
                Debug.LogWarning("Object already passed the ground or moving upward indefinitely.");
                return None;
            }

            // Use full 3D kinematic equation to compute the landing position
            Vector3 landingPosition =
                currentPosition +
                initialVelocity * timeUntilLanding +
                0.5f * gravity * timeUntilLanding * timeUntilLanding;

            return (timeUntilLanding, landingPosition);
        }

        public static float TimeToDestination(this Vector3 fromPos, Vector3 toPos, float maxSpeed)
            => maxSpeed == 0 ? Mathf.Infinity : Vector3.Distance(fromPos, toPos) / maxSpeed;

        public static float LookAtTimeToTarget(this GameObject fromObject, Vector3 toTarget, float degreesPerSecond)
            => fromObject.transform.LookAtTimeToTarget(toTarget, degreesPerSecond);
        public static float LookAtTimeToTarget(this Transform fromObject, Vector3 toTarget, float degreesPerSecond)
            => fromObject.LookAtTimeToDirection(toTarget - fromObject.position, degreesPerSecond);

        public static float LookAtTimeToDirection(this Transform fromObject, Vector3 targetDirection, float degreesPerSecond)
            => (targetDirection.sqrMagnitude < 0.0001f) ? 0f :
               Quaternion.Angle(fromObject.rotation, Quaternion.LookRotation(targetDirection.normalized)) / degreesPerSecond;


        public static (float, float) Scores(float distance, float velocity, uint difficulty)
        {
            (float, float) AD = GetSkillAD(difficulty);
            float A = AD.Item1;
            float D = AD.Item2;
            float vDiff = velocity - A;
            ...
            float velo = vDiff * velocity;
            float diff = distance - D;
            if (diff < 0)
            {
                diff = 0;
            }

            float dist = ((velocity / A) / A) + (diff / 7f);

            return (dist, velocity);
        }

        public static (float, float) GetSkillAD(uint difficulty)
        {
            switch (difficulty)
            {
                case 0: // SkillLevel.Rookie:
                    return (37.5f, 40f);
                case 1: // SkillLevel.Veteran:
                    return (53f, 55f);
                case 2: // SkillLevel.AllStar:
                    return (62f, 60f);
                case 3: // SkillLevel.HallOfFame:
                    return (68.5f, 70f);
            }
    ...
    ///from Math3d.cs @ https://gist.github.com/jankolkmeier/8543156
    public static partial class MathUtilities
    {
        ...
        public static int ToRoundedPercentage(this float val)
        {
